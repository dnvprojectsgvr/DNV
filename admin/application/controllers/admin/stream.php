...
class stream extends CI_Controller 
{
  ...
  {   
    ...
    if($this->input->post('submit'))
    {
      ...
      $this->db->insert('stream_details', $post_data);
      redirect('admin/stream/index');
    }


    $this->load->view('Admin/stream/add', $data);
  }


  public function index($stream_name = 'all', $sort_column = 'all', $sort_order = 'all', $page_no = 0)
  {
    $data['url'] = site_url('admin/stream/index/'.$stream_name);

    //REDIRECT IF FILTER IS ENABLED
    if($this->input->post('submit'))
    {
      $url = 'admin/stream/index/'.$this->input->post('stream_name');
      redirect($url);
    }

    $data['title'] = "View All Streams";
    ...
    $num_rows = $this->db->get('stream_details')->num_rows();
    ...
    $this->load->view('admin/stream/index', $data);
  }

  public function edit($id)
  {
    $data['title'] = "Edit Stream";
    $data['login_data'] =  $this->session->login_data;
    ...
    if($this->input->post('submit'))
    {
      $post_data = $this->input->post('data');
      $this->db->where('id', $id);
      $this->db->update('stream_details', $post_data);
      redirect('admin/stream/index');
    }

    $data['stream'] = $this->db->get_where('stream_details', array('id' => $id), 1, 0)->result_array();
    $this->load->view('admin/stream/edit', $data);
  }
