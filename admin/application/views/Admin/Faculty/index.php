        ...
        <div class="box-body">
          ...
          <div class="col-md-12 table-responsive">
            <table class="table table-bordered table-hover">
              ...
              <tbody>
              <?php for($i=0; $i<count($faculty_details); $i++) { ?>
              <tr>
                <td><?php echo $i+1; ?></td>
                <td><?php echo $faculty_details[$i]['full_name']; ?></td>
                <td><?php echo $faculty_details[$i]['faculty_id']; ?></td>
                <td><?php echo $faculty_details[$i]['email_id']; ?></td>
                <td><?php echo $faculty_details[$i]['mob_no']; ?></td>
                <td align="center">
                  <a href="<?php echo site_url('Faculty/detail/'.$faculty_details[$i]['id']); ?>" title="View" class="btn btn-primary btn-xs" ><i class="fa fa-eye"></i></a> |
                  <a href="<?php echo site_url('Faculty/edit/'.$faculty_details[$i]['id']); ?>" title="View" class="btn btn-warning btn-xs" target="_blank"><i class="fa fa-pencil"></i></a> | 
                  <a href="<?php echo site_url('Faculty/delete/'.$faculty_details[$i]['id']); ?>" title="Delete" class="btn btn-danger btn-xs"><i class="fa fa-trash-o" onclick="return confirm('Do you really want to delete all the data related with this Faculty?');"></i>
                  </a>          
                </td>
              </tr>
              <?php } ?>
...
<script type="text/javascript">
  $(document).on('change', '#result_limit', function(){
    ...
    window.location = "<?php echo site_url('student/view_faculty_details'); ?>/" + brand + '/' + stencil_no + '/' + $(this).val() + '/' + sort_column + '/' + sort_order + '/0'; 
  });
</script>
</body>
</html>
